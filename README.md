...
The Objectives prototype uses standalone HTML, CSS, JavaScript and committed direct Figma exports. Open `objectives-prototype.html` directly or run `python3 build.py` and serve `dist/`. The 1440px composition uses exact Figma surfaces and scales uniformly to fit smaller windows. Objectives uses native HTML text and table rows with SVG icons. Static surrounding surfaces use 2× retina exports. Both sidebars stay at viewport height, with the main-menu footer anchored to the bottom; only the central content scrolls. Only the Objectives section is interactive. Section state resets on reload. Reduced-motion preferences disable animation.
